...
function TimelineItem({ item, humanizeActivity, onEdit, onDelete, onRewind }: { item: AgentWorkbenchTimelineItem; humanizeActivity: (value: string) => string; onEdit?: (id: string, content: string) => void; onDelete?: (id: string) => void; onRewind?: (id: string) => void }): React.JSX.Element | null {
  ...
  const content = humanizeActivity(item.content)
  if (item.kind === 'user') return <article className="agent-message-row user"><div className="agent-message agent-message-user">{editing ? <div className="agent-message-edit"><textarea value={draftText} onChange={(event) => setDraftText(event.target.value)} rows={3} autoFocus /><div className="agent-message-edit-actions"><button type="button" className="agent-message-edit-confirm" onClick={() => { onEdit?.(item.id, draftText); setEditing(false) }}>保存</button><button type="button" className="agent-message-edit-cancel" onClick={() => setEditing(false)}>取消</button></div></div> : <p>{content}</p>}</div>{!editing && (onEdit || onDelete || onRewind) ? <div className="agent-message-actions">{onEdit ? <button type="button" title="编辑" aria-label="编辑" onClick={() => { setDraftText(item.content); setEditing(true) }}><Pencil size={12} /></button> : null}{onDelete ? <button type="button" title="删除" aria-label="删除" onClick={() => onDelete(item.id)}><Trash2 size={12} /></button> : null}{onRewind ? <button type="button" title="回退到此处" aria-label="回退到此处" onClick={() => onRewind(item.id)}><Undo2 size={12} /></button> : null}</div> : null}<time>{formatTime(item.time)}</time></article>
